...
export const login = async (req, res) => {
    ...
    try {
        ...
        if (!rows.lenght) return res.status(401).json({error: 'Credenciais inválidas'})
        const user = rows[0]
...
export const logout = async (req, res) => {
    ...
    export const forgotPassword = async (req, res) => {
        ...
        if (!email || !newPassword) return res.status(400).json({ error:
        'Envie email e newPassword' })
        try {
        const hashed = await bcrypt.hash(newPassword, 10)
        await db.query('UPDATE users SET password = ? WHERE email = ?',
        [hashed, email])
        // Mensagem genérica: não revela se e-mail existe
        return res.json({ message: 'Se o e-mail existir, a senha foi redefinida.' })
        } catch (err) {
